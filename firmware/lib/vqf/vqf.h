...
// Taken from https://github.com/dlaidig/vqf/tree/main/vqf/cpp/
 ...
 *
 * The parameters influence the behavior of the algorithm and are independent of the sampling rate
 * of the IMU data. The constructor sets all parameters to the default values.
 *
 * The parameters #motionBiasEstEnabled, #restBiasEstEnabled, and #magDistRejectionEnabled can be
 * used to enable/disable the main features of the VQF algorithm. The time constants #tauAcc and
 * #tauMag can be tuned to change the trust on the accelerometer and magnetometer measurements,
 * respectively. The remaining parameters influence bias estimation and magnetometer rejection.
 */
struct VQFParams {
    /**
     ...
     *
     * Small values for \f$\tau_\mathrm{acc}\f$ imply trust on the accelerometer measurements and
     * while large values of \f$\tau_\mathrm{acc}\f$ imply trust on the gyroscope measurements.
     *
     ...
     *
     * Small values for \f$\tau_\mathrm{mag}\f$ imply trust on the magnetometer measurements and
     * while large values of \f$\tau_\mathrm{mag}\f$ imply trust on the gyroscope measurements.
     *
     * The time constant \f$\tau_\mathrm{mag}\f$ corresponds to the cutoff frequency
     * \f$f_\mathrm{c}\f$ of the first-order low-pass filter for the heading correction as follows:
     ...
     *
     * If set to true (default), gyroscope bias is estimated based on the inclination correction
     * only, i.e. without using magnetometer measurements.
     */
     ...
     *
     * If set to true (default), phases in which the IMU is at rest are detected. During rest, the
     * gyroscope bias is estimated from the low-pass filtered gyroscope readings.
     */
     ...
     * magnetometer-based updates are performed, but with an increased time constant.
     */
    ...
    /**
     * @brief Time in which the bias estimation uncertainty increases from 0 °/s to 0.1 °/s (in
     * seconds).
     *
     ...
     *
     * This value is used to clip the bias estimate and the measurement error in the bias estimation
     * update step. It is further used by the rest detection algorithm in order to not regard
     * measurements with a large but constant angular rate as rest.
     *
     * Default value: 2.0 °/s
     */
    vqf_real_t biasClip;
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
    /**
     * @brief Standard deviation of the converged bias estimation uncertainty during motion (in
     * degrees per second).
     *
     * This value determines the trust on motion bias estimation updates. A small value leads to
     * fast convergence.
     *
     * Default value: 0.1 °/s
     */
     ...
     *
     * As magnetometer measurements are deliberately not used during motion bias estimation,
     * gyroscope bias is not observable in vertical direction. This value is the relative weight of
     * an artificial zero measurement that ensures that the bias estimate in the unobservable
     * direction will eventually decay to zero.
     *
...
#endif
    /**
     * @brief Standard deviation of the converged bias estimation uncertainty during rest (in
     * degrees per second).
     *
     * This value determines the trust on rest bias estimation updates. A small value leads to fast
     * convergence.
     *
     * Default value: 0.03 °
     */
     ...
     *
     * Rest is detected when the measurements have been close to the low-pass filtered reference for
     * the given time.
     *
     * Default value: 1.5 s
     */
     ...
     *
     * This time constant characterizes a second-order Butterworth low-pass filter used to obtain
     * the reference for rest detection.
     *
     * Default value: 0.5 s
     */
     ...
     *
     * For rest to be detected, the norm of the deviation between measurement and reference must be
     * below the given threshold. (Furthermore, the absolute value of each component must be below
     * #biasClip).
     *
     * Default value: 2.0 °/s
     */
    ...
    /**
     * @brief Time constant for current norm/dip value in magnetic disturbance detection (in
     * seconds).
     *
     * This (very fast) low-pass filter is intended to provide additional robustness when the
     * magnetometer measurements are noisy or not sampled perfectly in sync with the gyroscope
     * measurements. Set to -1 to disable the low-pass filter and directly use the magnetometer
     * measurements.
     *
    ...
    /**
     * @brief Threshold for the magnetic field dip angle for magnetic disturbance detection (in
     * degrees).
     *
     * Default vaule: 10 °
     */
     ...
     * within the thresholds #magNormTh and #magDipTh for the given time. Additionally, only phases
     * with sufficient movement, specified by #magNewMinGyr, count.
     *
     * Default value: 20.0
     */
    vqf_real_t magNewTime;
    /**
     * @brief Duration after which to accept a homogeneous magnetic field for the first time (in
     * seconds).
     *
    ...
    /**
     * @brief Minimum angular velocity needed in order to count time for new magnetic field
     * acceptance (in °/s).
     *
     * Durations for which the angular velocity norm is below this threshold do not count towards
     * reaching #magNewTime.
     *
     * Default value: 20.0 °/s
     */
    ...
    /**
     * @brief Minimum duration within thresholds after which to regard the field as undisturbed
     * again (in seconds).
     *
     * Default value: 0.5 s
     */
     ...
     *
     * For magnetic disturbances up to this duration, heading correction is fully disabled and
     * heading changes are tracked by gyroscope only. After this duration (or for many small
     * disturbed phases without sufficient time in the undisturbed field in between), the heading
     * correction is performed with an increased time constant (see #magRejectionFactor).
     *
     ...
     *
     * After #magMaxRejectionTime of full magnetic disturbance rejection, heading correction is
     * performed with an increased time constant. This parameter (approximately) specifies the
     * factor of the increase.
     *
     * Furthermore, after spending #magMaxRejectionTime/#magRejectionFactor seconds in an
     * undisturbed magnetic field, the time is reset and full magnetic disturbance rejection will be
     * performed for up to #magMaxRejectionTime again.
     *
     * Default value: 2.0
     */
 ...
 * VQF::getRestDetected() and VQF::getMagDistDetected(). To reset the state to the initial values,
 * use VQF::resetState().
 *
 * Direct access to the full state is typically not needed but can be useful in some cases, e.g. for
 * debugging. For this purpose, the state can be accessed by VQF::getState() and set by
 * VQF::setState().
 */
struct VQFState {
    /**
     ...
     *
     * Change to inclination correction quaternion \f$^{\mathcal{I}_i}_{\mathcal{E}_i}\mathbf{q}\f$
     ...
     *
     * This value is used as the gain for heading correction in the beginning if it is larger than
     * the normal filter gain. It is initialized to 1 and then updated to 0.5, 0.33, 0.25, ... After
     * VQFParams::tauMag seconds, it is set to zero.
     */
    vqf_real_t kMagInit;
    /**
     * @brief Last heading disagreement angle.
     *
     * Disagreement between the heading \f$\hat\delta\f$ estimated from the last magnetometer sample
     * and the state \f$\delta\f$ (in rad).
     */
     ...
     *
     * The 3x3 matrix is stored in row-major order. Note that for numeric reasons the internal unit
     * used is 0.01 °/s, i.e. to get the standard deviation in degrees per second use \f$\sigma =
     * \frac{\sqrt{p_{ii}}}{100}\f$.
     */
    vqf_real_t biasP[9];
#else
...
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
    /**
     * @brief Internal state of the Butterworth low-pass filter for the rotation matrix coefficients
     * used in motion bias estimation.
     */
    double motionBiasEstRLpState[9 * 2];
    /**
     * @brief Internal low-pass filter state for the rotated bias estimate used in motion bias
     * estimation.
     */
    double motionBiasEstBiasLpState[2 * 2];
#endif
    /**
     * @brief Last (squared) deviations from the reference of the last sample used in rest
     * detection.
     *
     ...
     *
     * The method VQF::getRelativeRestDeviations() provides an easier way to obtain and interpret
     * those values.
     */
    vqf_real_t restLastSquaredDeviations[2];
    /**
     * @brief The current duration for which all sensor readings are within the rest detection
     * thresholds.
     *
    ...
    /**
     * @brief Last low-pass filtered accelerometer measurement used as the reference for rest
     * detection.
     */
     ...
     *
     * If this value exceeds VQFParams::magNewTime (or VQFParams::magNewFirstTime if #magRefNorm <
     * 0), the current candidate is accepted as the new reference.
     */
 ...
 *
 * Coefficients are values that depend on the parameters and the sampling times, but do not change
 * during update steps. They are calculated in VQF::setup().
 */
struct VQFCoefficients {
    ...
    /**
     * @brief Measurement noise variance for the motion gyroscope bias estimation update in vertical
     * direction.
     */
    ...
    /**
     * @brief Numerator coefficients of the gyroscope measurement low-pass filter for rest
     * detection.
     */
    double restGyrLpB[3];
    /**
     * @brief Denominator coefficients of the gyroscope measurement low-pass filter for rest
     * detection.
     */
    double restGyrLpA[2];
    /**
     * @brief Numerator coefficients of the accelerometer measurement low-pass filter for rest
     * detection.
     */
    double restAccLpB[3];
    /**
     * @brief Denominator coefficients of the accelerometer measurement low-pass filter for rest
     * detection.
     */
    double restAccLpA[2];

    /**
     * @brief Gain of the first-order filter used for to update the magnetic field reference and
     * candidate.
     */
    vqf_real_t kMagRef;
    /**
 ...
 *
 *
 *     D. Laidig and T. Seel. "VQF: Highly Accurate IMU Orientation Estimation with Bias Estimation
 * and Magnetic Disturbance Rejection." Information Fusion 2023, 91, 187--204.
 ...
 *
 * The filter can perform simultaneous 6D (magnetometer-free) and 9D (gyr+acc+mag) sensor fusion and
 * can also be used without magnetometer data. It performs rest detection, gyroscope bias estimation
 * during rest and motion, and magnetic disturbance detection and rejection. Different sampling
 * rates for gyroscopes, accelerometers, and magnetometers are supported as well. While in most
 * cases, the defaults will be reasonable, the algorithm can be influenced via a number of tuning
 * parameters.
 *
 ...
 * 3. access the estimation results with :meth:`getQuat6D() <VQF.getQuat6D>`, :meth:`getQuat9D()
 * <VQF.getQuat9D>` and the other getter methods.
 *
 * If the full data is available in (row-major) data buffers, you can use :meth:`updateBatch()
 * <VQF.updateBatch>`.
 *
 * This class is the main C++ implementation of the algorithm. Depending on use case and programming
 * language of choice, the following alternatives might be useful:
 *
 * +------------------------+--------------------------+--------------------------+---------------------------+
...
public:
     ...
     *
     * In the most common case (using the default parameters and all data being sampled with the
     * same frequency, create the class like this: \rst
     * .. code-block:: c++
     *
     ...
     * @param accTs sampling time of the accelerometer measurements in seconds (the value of `gyrTs`
     * is used if set to -1)
     * @param magTs sampling time of the magnetometer measurements in seconds (the value of `gyrTs`
     * is used if set to -1)
     *
     */
     ...
     * is used if set to -1)
     */
    VQF(const VQFParams& params, vqf_real_t gyrTs, vqf_real_t accTs = -1.0,
        vqf_real_t magTs = -1.0);

    /**
     * @brief Performs gyroscope update step.
     *
     * It is only necessary to call this function directly if gyroscope, accelerometers and
     * magnetometers have different sampling rates. Otherwise, simply use #update().
     *
     ...
     *
     * In order to use this function, the input data must be available in an array in row-major
     * order. A null pointer can be passed for mag in order to skip the magnetometer update. All
     * data must have the same sampling rate.
     *
     * The output pointer arguments must be null pointers or point to sufficiently large data
     * buffers.
     *
     * Example usage:
     * \rst
     ...
     *
     *     VQF vqf(0.01); // 0.01 s sampling time, i.e. 100 Hz
     *     vqf.updateBatch(gyr, acc, mag, nullptr, quat9D, nullptr, nullptr, nullptr, nullptr,
     * nullptr); \endrst
     *
     ...
     * @param outDelta output buffer for heading difference angle (N elements, can be a null
     * pointer)
     * @param outBias output buffer for the gyroscope bias estimate (N*3 elements, can be a null
     * pointer)
     * @param outBiasSigma output buffer for the bias estimation uncertainty (N elements, can be a
     * null pointer)
     * @param outRest output buffer for the rest detection state (N elements, can be a null pointer)
     * @param outMagDist output buffer for the magnetic disturbance state (N elements, can be a null
     * pointer)
     */
    void updateBatch(
        const vqf_real_t gyr[], const vqf_real_t acc[], const vqf_real_t mag[], size_t N,
    ...
    /**
     * @brief Returns the heading difference \f$\delta\f$ between \f$\mathcal{E}_i\f$ and
     * \f$\mathcal{E}\f$.
     *
     * \f$^{\mathcal{E}_i}_{\mathcal{E}}\mathbf{q} = \begin{bmatrix}\cos\frac{\delta}{2} & 0 & 0 &
     * \sin\frac{\delta}{2}\end{bmatrix}^T\f$.
     ...
     *
     * The returned standard deviation sigma represents the estimation uncertainty in the worst
     * direction and is based on an upper bound of the largest eigenvalue of the covariance matrix.
     *
     ...
     * @param sigma standard deviation of the estimation uncertainty (rad/s) - set to -1 (default)
     * in order to not change the estimation covariance matrix
     */
     ...
     *
     * Looking at those values can be useful to understand how rest detection is working and which
     * thresholds are suitable. The output array is filled with the last values for gyroscope and
     * accelerometer, relative to the threshold. In order for rest to be detected, both values must
     * stay below 1.
     *
     ...
     *
     * This method allows to set a completely arbitrary filter state and is intended for debugging
     * purposes. In combination with #getState, individual elements of the state can be modified.
     *
    ...
    /**
     * @brief Performs quaternion multiplication (\f$\mathbf{q}_\mathrm{out} = \mathbf{q}_1 \otimes
     * \mathbf{q}_2\f$).
     */
     ...
     *
     * @param tau time constant \f$\tau\f$ in seconds - use -1 to disable update (\f$k=0\f$) or 0 to
     * obtain unfiltered values (\f$k=1\f$)
     ...
     *
     * The filter is parametrized via the time constant of the dampened, non-oscillating part of
     * step response and the resulting cutoff frequency is \f$f_\mathrm{c} =
     ...
     */
    static void
    filterInitialState(vqf_real_t x0, const double b[], const double a[], double out[2]);
    /**
     * @brief Adjusts the filter state when changing coefficients.
     *
     * This function assumes that the filter is currently in a steady state, i.e. the last input
     * values and the last output values are all equal. Based on this, the filter state is adjusted
     * to new filter coefficients so that the output does not jump.
     *
    ...
    static void filterAdaptStateForCoeffChange(
        vqf_real_t last_y[], size_t N, const double b_old[3], const double a_old[2],
     ...
     *
     * During the first \f$\tau\f$ seconds, the filter output is the mean of the previous samples.
     * At \f$t=\tau\f$, the initial conditions for the low-pass filter are calculated based on the
     * current mean value and from then on, regular filtering with the rational transfer function
     * described by the coefficients b and a is performed.
     *
    ...
    static void filterVec(
        const vqf_real_t x[], size_t N, vqf_real_t tau, vqf_real_t Ts, const double b[3],
     ...
     */
    static void
    matrix3Multiply(const vqf_real_t in1[9], const vqf_real_t in2[9], vqf_real_t out[9]);
    /**
     ...
     */
    static void
    matrix3MultiplyTpsFirst(const vqf_real_t in1[9], const vqf_real_t in2[9], vqf_real_t out[9]);
    /**
     ...
     */
    static void
    matrix3MultiplyTpsSecond(const vqf_real_t in1[9], const vqf_real_t in2[9], vqf_real_t out[9]);
    /**
...
protected:
     ...
     *
     * See #getParams. To set parameters, pass them to the constructor. Part of the parameters can
     * be changed with #setTauAcc, #setTauMag, #setMotionBiasEstEnabled, #setRestBiasEstEnabled,
     * #setMagDistRejectionEnabled, and #setRestDetectionThresholds.
     */
    VQFParams params;
    /**
