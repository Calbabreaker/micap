...
inline vqf_real_t square(vqf_real_t x) {
    return x * x;
}
...
#endif
      ,
      restBiasEstEnabled(true), magDistRejectionEnabled(true), biasSigmaInit(0.5),
      biasForgettingTime(100.0), biasClip(2.0)
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
      ,
      biasSigmaMotion(0.1), biasVerticalForgettingFactor(0.0001)
#endif
      ,
      biasSigmaRest(0.03), restMinT(1.5), restFilterTau(0.5), restThGyr(2.0), restThAcc(0.5),
      magCurrentTau(0.05), magRefTau(20.0), magNormTh(0.1), magDipTh(10.0), magNewTime(20.0),
      magNewFirstTime(5.0), magNewMinGyr(20.0), magMinUndisturbedTime(0.5),
      magMaxRejectionTime(60.0), magRejectionFactor(2.0) {
}
...
void VQF::updateGyr(const vqf_real_t gyr[3]) {
    ...
    if (params.restBiasEstEnabled || params.magDistRejectionEnabled) {
        filterVec(
            gyr, 3, params.restFilterTau, coeffs.gyrTs, coeffs.restGyrLpB, coeffs.restGyrLpA,
    ...
    vqf_real_t gyrNoBias[3] = {
        gyr[0] - state.bias[0], gyr[1] - state.bias[1], gyr[2] - state.bias[2]
    };
...
void VQF::updateAcc(const vqf_real_t acc[3]) {
    ...
    if (params.restBiasEstEnabled) {
        filterVec(
            acc, 3, params.restFilterTau, coeffs.accTs, coeffs.restAccLpB, coeffs.restAccLpA,
    ...
    filterVec(
        accEarth, 3, params.tauAcc, coeffs.accTs, coeffs.accLpB, coeffs.accLpA, state.accLpState,
        state.lastAccLp
    );
    ...
    if (q_w > 1e-6) {
        ...
        // to avoid numeric issues when acc is close to [0 0 -1], i.e. the correction step is close
        // (<= 0.00011°) to 180°:
        accCorrQuat[0] = 0;
        accCorrQuat[1] = 1;
        accCorrQuat[2] = 0;
    ...
    if (params.motionBiasEstEnabled || params.restBiasEstEnabled) {
        ...
        filterVec(
            R, 9, params.tauAcc, coeffs.accTs, coeffs.accLpB, coeffs.accLpA,
            state.motionBiasEstRLpState, R
        );
        filterVec(
            biasLp, 2, params.tauAcc, coeffs.accTs, coeffs.accLpB, coeffs.accLpA,
            state.motionBiasEstBiasLpState, biasLp
        );
...
#else
    // simplified implementation of bias estimation for the special case in which only rest bias
    ...
    if (params.restBiasEstEnabled) {
        ...
        if (state.restDetected) {
            ...
            // (this version only uses the first entry of P as P is diagonal and all diagonal
            // elements are the same) step 1: P = P + V (done above!) step 2: K = P R^T inv(W + R P
            // R^T)
...
void VQF::updateMag(const vqf_real_t mag[3]) {
    ...
    if (params.magDistRejectionEnabled) {
        ...
        if (params.magCurrentTau > 0) {
            filterVec(
                state.magNormDip, 2, params.magCurrentTau, coeffs.magTs, coeffs.magNormDipLpB,
    ...
    if (params.magDistRejectionEnabled) {
        ...
        if (state.magDistDetected) {
            ...
            state.magRejectT = std::max(
                state.magRejectT - params.magRejectionFactor * coeffs.magTs, vqf_real_t(0.0)
            );
        }
    }
...
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
    // use largest absolute row sum as upper bound estimate for largest eigenvalue (Gershgorin
    // circle theorem) and clip output to biasSigmaInit
...
void VQF::getRelativeRestDeviations(vqf_real_t out[2]) const {
    out[0] =
        sqrt(state.restLastSquaredDeviations[0]) / (params.restThGyr * vqf_real_t(M_PI / 180.0));
...
void VQF::setTauAcc(vqf_real_t tauAcc) {
    ...
    filterAdaptStateForCoeffChange(
        state.lastAccLp, 3, coeffs.accLpB, coeffs.accLpA, newB, newA, state.accLpState
    );
    ...
    filterAdaptStateForCoeffChange(
        R, 9, coeffs.accLpB, coeffs.accLpA, newB, newA, state.motionBiasEstRLpState
    );
    vqf_real_t biasLp[2];
    ...
    filterAdaptStateForCoeffChange(
        biasLp, 2, coeffs.accLpB, coeffs.accLpA, newB, newA, state.motionBiasEstBiasLpState
    );
#endif
...
void VQF::quatRotate(const vqf_real_t q[4], const vqf_real_t v[3], vqf_real_t out[3]) {
    vqf_real_t x = (1 - 2 * q[2] * q[2] - 2 * q[3] * q[3]) * v[0] +
                   2 * v[1] * (q[2] * q[1] - q[0] * q[3]) + 2 * v[2] * (q[0] * q[2] + q[3] * q[1]);
    vqf_real_t y = 2 * v[0] * (q[0] * q[3] + q[2] * q[1]) +
                   v[1] * (1 - 2 * q[1] * q[1] - 2 * q[3] * q[3]) +
                   2 * v[2] * (q[2] * q[3] - q[1] * q[0]);
    vqf_real_t z = 2 * v[0] * (q[3] * q[1] - q[0] * q[2]) + 2 * v[1] * (q[0] * q[1] + q[3] * q[2]) +
...
void VQF::filterCoeffs(vqf_real_t tau, vqf_real_t Ts, double outB[], double outA[]) {
    ...
    double fc = (M_SQRT2 / (2.0 * M_PI)) /
                double(tau); // time constant of dampened, non-oscillating part of step response
...
void VQF::filterInitialState(vqf_real_t x0, const double b[3], const double a[2], double out[]) {
    // initial state for steady state (equivalent to scipy.signal.lfilter_zi, obtained by setting
    // y=x=x0 in the filter update equation)
    out[0] = x0 * (1 - b[0]);
    out[1] = x0 * (b[2] - a[1]);
}

void VQF::filterAdaptStateForCoeffChange(
    vqf_real_t last_y[], size_t N, const double b_old[], const double a_old[], const double b_new[],
...
void VQF::matrix3MultiplyTpsFirst(
    const vqf_real_t in1[9], const vqf_real_t in2[9], vqf_real_t out[9]
) {
    vqf_real_t tmp[9];
...
void VQF::matrix3MultiplyTpsSecond(
    const vqf_real_t in1[9], const vqf_real_t in2[9], vqf_real_t out[9]
) {
    vqf_real_t tmp[9];
...
void VQF::setup() {
    ...
    coeffs.biasVerticalW =
        coeffs.biasMotionW / std::max(params.biasVerticalForgettingFactor, vqf_real_t(1e-10));
#endif
    ...
    if (params.magCurrentTau > 0) {
        filterCoeffs(
            params.magCurrentTau, coeffs.magTs, coeffs.magNormDipLpB, coeffs.magNormDipLpA
        );
    } else {
